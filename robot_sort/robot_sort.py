class SortingRobot:
    ...
    def sort(self):
        ...
        if not self.light_is_on():
            ...
            # if the robot can't move right, the list is sorted, and we are done.
            elif not self.can_move_right():
                # print('We are done?')
                # print(self._time)
                return
            
            # we just finished sorting the item in front of us, lets move right one item and pick it up
            else:
                self.swap_item()
                self.set_light_on()
                self.sort()
